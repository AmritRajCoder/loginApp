import React, { Component } from 'react';//Component is like a component
import { StyleSheet, Text, ScrollView, TextInput, View, ActivityIndicator } from 'react-native';
import BasicButton from './BasicButton';
import LoginSignUpBtn from './BasicButton';
...
export default class SignUp extends ValidationComponent {
   ...
   playAudio = async () => {
      try {
         const soundObject = new Audio.Sound();//with audio class create new sound component
         await soundObject.loadAsync(require('../assets/ding2.mp3'));//load the file with await to stop everything else and run this part, require gets the file

         await soundObject.playAsync();//play sound
   ...
   handleRegisterBtnClick = () => {
      ...
      if (this.getErrorMessages()) {//gets all error messages
         this.displaySnackBar("error", this.getErrorMessages());
      } else {
         firebase.auth().createUserWithEmailAndPassword(this.state.email, this.state.password)
            .then((user) => {//DOUBT BOUBT
               this.hideSnackBar();
               this.playAudio();//predefined
            ...
            .catch((error) => {
               ...
               this.hideLoader();
               this.hideSnackBar();
               this.displaySnackBar("error", errorMessage, errorCode);//third argument not accepted
            });
         setTimeout(() => {
            this.hideLoader();
         }, 1000);
   ...
   displaySnackBar = (type, text) => {
      this.setState({
         "snackBarType": type,
         "snackBarText": text,
      }); //DOUBT DOUBT - we can merge it in one
      this.setState({
